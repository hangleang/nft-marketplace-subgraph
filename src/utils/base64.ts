...
export function isBase64(encoded: string): bool {
  return encoded.startsWith(BASE64_PREFIX);
}

export function base64ToJSON(encoded: string): JSONValue | null {
	if (isBase64(encoded)) {
...
// below are code from https://github.com/near/as-base64/blob/master/assembly/index.ts
const PADCHAR = "=";
const ALPHA =
...
export function encode(bytes: Uint8Array): string {
  ...
  let x = changetype<string>((<usize>(len << 1), idof<string>()));
